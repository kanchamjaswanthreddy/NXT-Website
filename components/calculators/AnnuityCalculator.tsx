...
export default function AnnuityCalculator() {
  const [investment, setInvestment] = useState(250000); const [rate, setRate] = useState(5); const [payout, setPayout] = useState(20); const [deferral, setDeferral] = useState(5)
  const accumulated = investment * Math.pow(1 + rate / 100, deferral)
  const r = rate / 100 / 12; const n = payout * 12
  ...
  return (
    <div className="card rounded-[24px] p-7 md:p-8">
      <h3 className="display-sm">Annuity payout estimator</h3><p className="mb-6 mt-1 text-[15px] text-ink-soft">See how deferral and growth rate affect your monthly retirement income.</p>
      ...
      <dl className="mono mt-6 grid grid-cols-2 gap-4 text-sm"><div><dt className="text-ink-soft">Accumulated value</dt><dd className="text-navy">{usd(Math.round(accumulated))}</dd></div><div><dt className="text-ink-soft">Total payouts</dt><dd className="text-navy">{usd(monthly * n)}</dd></div></dl>
