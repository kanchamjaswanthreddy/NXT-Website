...
export default function FourOhOneKCalculator() {
  const [age, setAge] = useState(30); const [retireAge, setRetireAge] = useState(65); const [salary, setSalary] = useState(80000); const [contrib, setContrib] = useState(10); const [match, setMatch] = useState(4); const [balance, setBalance] = useState(25000)
  const years = Math.max(0, retireAge - age); const monthlyContrib = (salary * (contrib + Math.min(match, contrib)) / 100) / 12; const r = 0.07 / 12; const n = years * 12
  const future = Math.round(balance * Math.pow(1 + r, n) + monthlyContrib * ((Math.pow(1 + r, n) - 1) / r))
  const totalContrib = Math.round(balance + salary * (contrib / 100) * years); const totalMatch = Math.round(salary * (Math.min(match, contrib) / 100) * years)
  return (
    <div className="card rounded-[24px] p-7 md:p-8">
      <h3 className="display-sm">401(k) projector</h3><p className="mb-6 mt-1 text-[15px] text-ink-soft">Estimate your balance at retirement with employer matching.</p>
      ...
      <dl className="mono mt-6 grid grid-cols-2 gap-4 text-sm"><div><dt className="text-ink-soft">Your contributions</dt><dd className="text-navy">{usd(totalContrib)}</dd></div><div><dt className="text-ink-soft">Employer match total</dt><dd className="text-navy">{usd(totalMatch)}</dd></div></dl>
