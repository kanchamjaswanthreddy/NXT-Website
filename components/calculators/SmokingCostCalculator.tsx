...
export default function SmokingCostCalculator() {
  const [packs, setPacks] = useState(1); const [price, setPrice] = useState(10); const [years, setYears] = useState(20)
  const annual = packs * price * 365; const totalSpent = Math.round(annual * years)
  const r = 0.07 / 12; const n = years * 12; const monthlyContrib = annual / 12
  const invested = Math.round(monthlyContrib * ((Math.pow(1 + r, n) - 1) / r))
  return (
    <div className="card rounded-[24px] p-7 md:p-8">
      <h3 className="display-sm">Cost of smoking</h3><p className="mb-6 mt-1 text-[15px] text-ink-soft">What your smoking habit costs outright, and what that money could become if invested instead.</p>
      ...
      <dl className="mono mt-6 grid grid-cols-2 gap-4 text-sm"><div><dt className="text-ink-soft">Total spent</dt><dd className="text-navy">{usd(totalSpent)}</dd></div><div><dt className="text-ink-soft">Daily cost</dt><dd className="text-navy">{usd(packs * price)}</dd></div></dl>
