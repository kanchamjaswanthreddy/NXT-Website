  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
      <div className="card rounded-[24px] p-7 md:p-8">
        ...
        <dl className="mono mt-6 grid grid-cols-3 gap-4 text-sm">
          <div><dt className="text-ink-soft">1st doubling</dt><dd className="text-navy">{usd(investment * 2)}</dd></div>
          <div><dt className="text-ink-soft">2nd doubling</dt><dd className="text-navy">{usd(investment * 4)}</dd></div>
          <div><dt className="text-ink-soft">3rd doubling</dt><dd className="text-navy">{usd(tripled)}</dd></div>
        </dl>
      ...
      <CalcChart
        ...
        xKey="year"
        xLabel="Years"
        series={[{ key: 'value', label: 'Investment value', color: '#C8A951' }]}
      />
    </div>
  )
}
